...
cuboid = []
inputTexts = input("請輸入長方體長寬高: ").split()
for text in inputTexts:
    cuboid.append(int(text))

# 也可改成下列寫法
# 1. 類似for-in loop
# cuboid = [int(numberStr) for numberStr in input("請輸入長方體長寬高: ").split()]
# 2. 使用集合函式，第1個參數int為函式: int(number)，可將每個元素轉成整數
# cuboid = list(map(int, input("請輸入長方體長寬高: ").split()))
